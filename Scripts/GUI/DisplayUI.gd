...
func _on_pause_ui_level_selection_visibility_changed() -> void:
	if $PauseButton:
		$PauseButton.visible = $PauseButton.visible

func _on_ui_event_visible_change():
	if $DebugUI/PauseButton:
		$DebugUI/PauseButton.visible = !$DebugUI/PauseButton.visible
	if $DebugUI/RestartButton:
		$DebugUI/RestartButton.visible = !$DebugUI/RestartButton.visible
